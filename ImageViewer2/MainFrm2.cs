...
namespace ImageViewer2
{
    public partial class MainFrm2 : Form
    {
        ...
        public MainFrm2()
        {
            InitializeComponent();
            isFrist = true;
            pictureBoxEx1.ContextMenuStrip = contextMenuStrip2;
        }
        public MainFrm2(string imgPath)
        {
            InitializeComponent();
        ...
        {
            pictureBoxEx1.IsAutoDispose = true;
        }
        ...
        private void pictureBoxEx1_MouseMove(object sender, MouseEventArgs e)
        {
            if (pictureBoxEx1.Image != null)
            {
                if (e.X <= 5 + 32 || e.X >= pictureBoxEx1.Width - 5 - 32)
                {
                    if (!canShowBtn)
                    {
                        Cursor = Cursors.Hand;
                        canShowBtn = true;
                        pictureBoxEx1.Refresh();
                    }
                }
                else
                {
                ...
                if (e.X >= 0 && e.Y >= 0)
                {
                    Point p = pictureBoxEx1.GetImagePoint(new Point(e.X, e.Y));
                    if (p.X >= 0 && p.Y >= 0 && p.X < pictureBoxEx1.Image.Width && p.Y < pictureBoxEx1.Image.Height)
                    {
                        Bitmap bitmap = (Bitmap)(pictureBoxEx1.Image);
        ...
        private void pictureBoxEx1_Paint(object sender, PaintEventArgs e)
        {
            if (pictureBoxEx1.Image != null)
            {
                if (canShowBtn)
                {
                    ...
                    g.DrawImage(Properties.Resources.左箭头, new Point(5, (pictureBoxEx1.Height - 32) / 2));
                    g.DrawImage(Properties.Resources.右箭头, new Point(pictureBoxEx1.Width - 32 - 5, (pictureBoxEx1.Height - 32) / 2));
                }
            }
        }

        private void pictureBoxEx1_MouseClick(object sender, MouseEventArgs e)
        {
            if (pictureBoxEx1.Image != null)
            {
                if (e.X <= 5 + 32)
                ...
                else if (e.X >= pictureBoxEx1.Width - 5 - 32)
                {
                    NextImg();
                }
            }
        ...
        private void pictureBoxEx1_MouseEnter(object sender, EventArgs e)
        {
            this.Activate();
        }
        ...
        private void 另存为ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (sfd.ShowDialog() == DialogResult.OK)
            {
                ...
                if (ext == ".bmp")
                {
                    pictureBoxEx1.Image.Save(sfd.FileName, System.Drawing.Imaging.ImageFormat.Bmp);
                }
                else if (ext == ".jpg" || ext == ".jpeg")
                {
                    pictureBoxEx1.Image.Save(sfd.FileName, System.Drawing.Imaging.ImageFormat.Jpeg);
                }
                else if (ext == ".png")
                {
                    pictureBoxEx1.Image.Save(sfd.FileName, System.Drawing.Imaging.ImageFormat.Png);
                }
                else if (ext == ".tif")
                {
                    pictureBoxEx1.Image.Save(sfd.FileName, System.Drawing.Imaging.ImageFormat.Tiff);
                }
               
            }
        }

        private void 右旋转ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            pictureBoxEx1.Rotate(90);
        }

        private void 左旋转ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            pictureBoxEx1.Rotate(-90);
        }
    }
